...
def solve_wing(x):
    ...
    #     # BUCKLING CHECK
    #     # ------------------------------------
    #     if tag in SKIP_BUCKLING:
    #         buckling_val = 0.0   # root rib: BC artifact, skip
    #     else:
    #         buckling_val = panel_buckling_index(
    #             eps_cells, mat, panel_width,
    ...
    for tag, mat in [
        ...
        *[(t, MAT_RIB) for t in TAG_RIBS]
        ...
        if len(cells) > 0:
            eps_cells = eps0_all[cells]
            kap_cells = kappa_all[cells]
            fi_val       = _eval_failure_from_arrays(eps_cells, kap_cells, mat, CE_STRENGTH, criterion="tsai_wu", label=f"TAG_{tag}")
            panel_width  = RIB_SPACING if tag in [TAG_UPPER, TAG_LOWER] else \
                        SPAR_HEIGHT if tag in [TAG_MAINSPAR, TAG_TAILSPAR] else RIB_PANEL
            buckling_val = 0.0 if tag in SKIP_BUCKLING else \
                        panel_buckling_index(eps_cells, mat, panel_width, buckling_k=K_MAP.get(tag, 4.0))
            combined = max(fi_val, buckling_val)
        else:
            fi_val = buckling_val = combined = 0.0

        # ← allreduce TRONG vòng lặp, per tag
        global_fi_tag    = comm.allreduce(combined,     op=MPI.MAX)
        global_tsaiwu    = comm.allreduce(fi_val,       op=MPI.MAX)
        global_buckling  = comm.allreduce(buckling_val, op=MPI.MAX)
        local_fi_max     = max(local_fi_max, global_fi_tag)

        if rank == 0:
            print(f"[TAG_{tag}] TsaiWu={global_tsaiwu:.4f} Buckling={global_buckling:.4f}")

    # Không cần allreduce nữa vì local_fi_max đã là global
    global_fi_max = local_fi_max
    ...
    if rank == 0:
        ...
        status = "SAFE" if global_fi_max < 1.005 else "FAIL"
...
if QUICK_TEST:
    ...
    if rank == 0:
        ...
        try:
            ...
            vprint(f"\n[RESULT] Skin={xopt[0]*1000:.2f}mm, Rib={xopt[1]*1000:.2f}mm, Spar={xopt[2]*1000:.2f}mm")
        finally:
