...
from dolfinx.mesh import compute_midpoints, exterior_facet_indices
...
RIB_BND   = f"rib_{RIB_INDEX}_boundary.npy"   # from RibMeshExtractor_STL.py
FOAM_XDMF = "FOAMData.xdmf"

VOLFRAC   = 0.55
P_START   = 1.0
P_MAX     = 4.0
P_RAMP    = 150      # iters to ramp penalty from P_START to P_MAX
LAME_NU   = 0.3
E0        = 210e9
FILTER_R  = 0.03    # [m] sensitivity filter radius
MOVE      = 0.05
MAX_ITER  = 10000
TOL       = 5e-3    # converge when grey elements < TOL fraction
...
lame_lps = 2 * lame_lam * lame_mu / (lame_lam + 2 * lame_mu)   # plane stress
...
x_le      = pts_np[:, 0].min()
thickness = pts_np[:, 2].max() - pts_np[:, 2].min()
print(f"Chord = {chord*1e3:.0f} mm,  Thickness = {thickness*1e3:.1f} mm")

# ── BOUNDARY CONDITIONS (spar at 25% and 75% chord) ──────────────────
...
# ── PASSIVE FLANGE ELEMENTS ───────────────────────────────────────────
# Use boundary from extraction file → correct XZ coords
# bnd      = np.load(RIB_BND)          # (N, 3)
# bnd_xz   = bnd[:, [0, 2]]            # project to XZ plane
...
bnd_verts  = np.unique(np.concatenate([f_to_v.links(f) for f in bnd_facets]))
bnd_xz     = domain.geometry.x[bnd_verts][:, [0, 2]]

# # Extract boundary vertices from mesh topology
# domain.topology.create_connectivity(fdim, 0)
# bnd_facets = exterior_facet_indices(domain.topology)
# f_to_v     = domain.topology.connectivity(fdim, 0)
# bnd_verts  = np.unique(np.concatenate([f_to_v.links(f) for f in bnd_facets]))
# bnd_xz     = domain.geometry.x[bnd_verts][:, [0, 2]]
# print(f"Boundary from mesh topology: {len(bnd_xz)} vertices")

mid_xz      = midpoints[:, [0, 2]]
dist_to_bnd = KDTree(bnd_xz).query(mid_xz)[0]
...
FLANGE_T     = 0.02 * thickness      # 5% of local thickness
passive_mask = dist_to_bnd < FLANGE_T
print(f"FLANGE_T = {FLANGE_T*1e3:.1f}mm")
print(f"Passive flange elements: {passive_mask.sum()} / {num_cells}")

# Sau khi tính passive_mask:
free_mask    = ~passive_mask
n_free       = free_mask.sum()
vol_passive  = cell_volumes[passive_mask].sum() / total_volume
print(f"Passive volume fraction: {vol_passive:.3f}")
print(f"Free cells: {n_free}")

# Target volume fraction trên free elements
# Total target = VOLFRAC → free target = (VOLFRAC - vol_passive) / (1 - vol_passive)
VOLFRAC_FREE = max(0.01, (VOLFRAC - vol_passive) / (1 - vol_passive))
print(f"VOLFRAC_FREE = {VOLFRAC_FREE:.3f}")

# Khởi tạo x_var chỉ cho free elements
x_var = np.zeros(num_cells)
x_var[passive_mask] = 1.0
x_var[free_mask]    = VOLFRAC_FREE   # uniform init trên free cells
...
L_compiled = form(dot(f_func, v_test) * ds)
...
with io.VTKFile(MPI.COMM_WORLD,
    ...
    for it in range(MAX_ITER):
        ...
        ocp_free = x_var[free_mask] * np.sqrt(-sensitivity[free_mask] / cell_volumes[free_mask])
        l1, l2   = 0.0, float(ocp_free.sum())
        while (l2 - l1) > 1e-6:
            lmid      = 0.5 * (l1 + l2)
            x_new_free = (ocp_free / lmid).clip(
                x_var[free_mask] - MOVE,
                x_var[free_mask] + MOVE
            ).clip(0.001, 1.0)
            vol = (x_new_free * cell_volumes[free_mask]).sum() / (total_volume * (1 - vol_passive))
            l1, l2 = (lmid, l2) if vol > VOLFRAC_FREE else (l1, lmid)

        x_new = x_var.copy()
        x_new[free_mask]    = x_new_free
        x_new[passive_mask] = 1.0


        # 8. Enforce passive flange
        # x_new[passive_mask] = 1.0
        ...
        change = np.max(np.abs(x_new - x_var))
        x_var  = 0.3 * x_new + 0.3 * x_var   # damped update
        x_var[passive_mask] = 1.0  
        ...
        # if change < 1e-4 and it > P_RAMP:
        if change < 5e-4 and it > P_RAMP:
