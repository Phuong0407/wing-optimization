...
# =============================================
# 1. MATERIAL PARAMETERS (giữ nguyên)
# =============================================
thick    = Constant(0.01)
E        = Constant(210e9)
rho      = Constant(7850.0)
...
# =============================================
# 2. LOAD FENICS MESH — PHẢI LÊN TRƯỚC
# =============================================
mesh = Mesh()
...
f = rho * thick * g   # gravity — định nghĩa sau khi có mesh

# =============================================
# 3. LOAD TRACTION — SAU KHI CÓ FENICS MESH
# =============================================
mapped_data   = meshio.read("MappedTraction.xdmf")
traction_vals = mapped_data.point_data["traction"]  # (N, 3)
...
# =============================================
# 5. LOCAL FRAME + FUNCTION SPACES (giữ nguyên)
# =============================================
...
# =============================================
# 6. KINEMATICS (giữ nguyên)
# =============================================
def hstack(vectors):
    return as_matrix([[v[i] for i in range(len(v))] for v in vectors]).T

P_plane = hstack([e1, e2])

def t_grad(u):
    return dot(grad(u), P_plane)
...
# =============================================
# 7. BCs, WEAK FORM, SOLVE
# =============================================
zero     = Constant((0.0, 0.0, 0.0))
bc_u     = DirichletBC(V.sub(0), zero, facets, 11)
bc_t     = DirichletBC(V.sub(1), zero, facets, 11)
bcs      = [bc_u, bc_t]
...
Wext = dot(f, u_) * dx + dot(t_load, u_) * dx  # gravity + aero

print("ndofs =", a.size(0))
...
theta_sol = v.sub(1, True)
theta_sol.rename("Rotation", "theta")

ffile.write(u_sol, 0)
ffile.write(theta_sol, 0)
ffile.close()
